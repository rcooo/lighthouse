...
namespace Nuwave\Lighthouse\Federation\Schema;
...
class SchemaPrinter
{
     ...
     * @api
     */
    public static function doPrint(Schema $schema, DocumentAST $document, array $options = []) : string
    {
     ...
     * @param array<string, bool> $options
     */
    protected static function printFilteredSchema(Schema $schema, callable $directiveFilter, callable $typeFilter, DocumentAST $document, array $options) : string
    {
            ...
            implode(
                ...
                array_filter(
                    array_merge(
                        ...
                        array_map(
                            static function ($type) use ($document, $options) : string {
                                return static::printType($type, $document, $options);
                            },
                            $types
                        )
                    )
     ...
     * @param array<string, bool> $options
     */
    public static function printType(Type $type, DocumentAST $document, array $options = []) : string
    {
        ...
        if ($type instanceof ObjectType) {
            return static::printObject($type, $document, $options);
        }
     ...
     * @param array<string, bool> $options
     */
    protected static function printObject(ObjectType $type, DocumentAST $document, array $options) : string
    {
        ...
        return static::printDescription($options, $type) .
            sprintf("type %s%s%s {\n%s\n}", $type->name, $implementedInterfaces, static::printFederatedType($type, $document), static::printFields($options, $type, $document));
    }

    /**
     * @param array<string, bool> $options
     */
    protected static function printFields(array $options, $type, DocumentAST $document) : string
    {
        ...
        return implode(
            "\n",
            array_map(
                static function ($f, $i) use ($type, $options, $document) : string {
                    return /*static::printDescription($options, $f, '  ', ! $i) . '  ' .*/
                        $f->name . static::printArgs($options, $f->args, '  ') . ': ' .
                        (string) $f->getType() . static::printDeprecated($f) . static::printFederatedField($type, $f, $document);
                },
                $fields,
                array_keys($fields)
            )
    ...
    protected static function printFederatedField($type, $fieldOrEnumVal, DocumentAST $document) : string
    {
        $directives = '';
        $typeDefinition = $document->types[$type->name];
        foreach ($typeDefinition->fields as $fieldNode) {
            if ($fieldNode->name->value === $fieldOrEnumVal->name) {
                foreach ($fieldNode->directives as $directive) {
                    if ($directive->name->value === 'external') {
                        $directives .= ' @external';
                    }
                    if ($directive->name->value === 'key') {
                        $directives .= sprintf(' @key(fields: "%s")', 'x');
                    }
                }
            }
        }
    ...
    protected static function printFederatedType($type, DocumentAST $document) : string
    {
        $directives = '';
        $typeDefinitionNode = $document->types[$type->name];
        foreach ($typeDefinitionNode->directives as $directive) {
            if ($directive->name->value === 'extends') {
                $directives .= ' @extends';
            }
            if ($directive->name->value === 'key') {
                $directives .= sprintf(' @key(fields: "%s")', $directive->arguments[0]->value->value);
            }
        }
